...
//we may have to store the sidebar hooks inside a store as well, since turns out the navbar randomly needing it will require
...
export default create<SiteInfo>()(
  persist(
    (set, get) => ({
      latestWPM: 0,
      ...
      calculateWPM: (timeInSeconds) => {
        ...
        count += get().correctPrefixLen(
          extraWord.trim(),
          wordArr[line][wordIndex]
        );
        console.log(count);
